alter table public.financial_notes
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'financial_notes_currency_check'
      and conrelid = 'public.financial_notes'::regclass
  ) then
    alter table public.financial_notes
    add constraint financial_notes_currency_check
    check (currency in ('UYU', 'USD', 'ARS', 'EUR'));
  end if;
end;
$$;

drop function if exists public.confirm_financial_note(
  uuid,
  uuid,
  text,
  text,
...
drop function if exists public.save_financial_note_draft(
  uuid,
  uuid,
  text,
  text,
...
begin
  ...
  values (
    ...
    p_user_id,
    p_folder,
    coalesce(nullif(upper(trim(p_currency)), ''), 'UYU'),
    p_title,
    p_body,
    p_created_at,
    p_updated_at,
