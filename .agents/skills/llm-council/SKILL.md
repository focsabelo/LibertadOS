---
name: llm-council
description: Use only when the user explicitly asks to run a council, war room, multi-agent debate, pressure test, or multiple independent advisors on a meaningful decision. Do not infer this from ordinary “should I,” “which option,” validation, review, or trade-off questions; do not use for factual lookups, simple decisions, or creation tasks.
---
...
This is adapted from Andrej Karpathy's LLM Council. He dispatches queries to multiple models, has them peer-review each other anonymously, then a chairman produces the final answer. In Codex, use parallel subagents with different thinking lenses instead of different models.
...
**A. Scan the workspace for context.** The user's question is often just the tip of the iceberg. The workspace may contain files that would dramatically improve the council's output. Before framing, quickly scan for and read any relevant context files:


- `AGENTS.md`, `PRODUCT.md`, or other project-context files in the workspace (business context, preferences, constraints)
...
- **The verdict presentation matters.** Keep the in-chat Markdown concise and scannable; do not create HTML unless the user explicitly asks for it.
