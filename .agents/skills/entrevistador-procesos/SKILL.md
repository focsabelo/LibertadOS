...
description: Use when the user explicitly asks for a discovery interview, requirements elicitation, or process definition, or when a complex workflow/automation request has unresolved decisions that would materially change what gets built. Do not use for well-scoped implementation, bug fixes, copy edits, narrow UI changes, or tasks whose missing details can be handled with safe assumptions.
---
